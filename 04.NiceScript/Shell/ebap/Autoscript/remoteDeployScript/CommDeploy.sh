...
# sh ./CommDeploy.sh "stop" $app $VersionDate
#[脚本说明] :
#1、常规jar应用部署(全量部署)包含如下流程:
# 1.1 应用服务器软件包备份、移除 
# 1.2 应用服务器停服务 
...
#基于传参获得软件名称(不带版本和扩展名),例：cust
APP_FILENAME=`echo $2 | awk -F"-" '{print $1}'`.jar 
#应用主目录,例：#/data/home/app/deploy/cust
APP_HOME=${ifspHome}/deploy/${APP_NAME}
...
if [ "$1" = "sysinfo" ]; then
	...
	#启动服务
	echo "nohup ${java} ${JAVA_MEM_OPTS} -Dappname=${APP_FILENAME} -Dfile.encoding=${CHARSET} -classpath ${CLASSPATH}:${RUM_MAIN} ${IfspRun}"
	...
	sleep 1
	#启动检查服务当前进程
	PIDS_NOW=`ps -ef | grep "${RUM_MAIN} ${IfspRun}" | grep -v "grep" | awk '{print $2}'`
	if [ -n "$PIDS_NOW" ]; then
		echo "[INFO]: 应用[${APP_FILENAME}]已运行，服务进程ID:${PIDS_NOW}";		
	else 
		echo "[ERROR]: 应用[${APP_FILENAME}]服务启动失败,程序退出."
		exit 1
	fi
	...
	if [ -e ${APP_HOME}/${APP_FILENAME}.JksBak_$3 ]; then
		...
		is_suc "mv ${APP_HOME}/${APP_FILENAME} ${APP_HOME}/${APP_FILENAME}.JksBak_$3"
	fi  
elif [ "$1" = "update" ]; then
	#将待发布版本替换到版本发布目录下
	RUM_MAIN=${APP_HOME}/${APP_FILENAME}
	cp ~/JenkinsAutodeploy/update/$3/jar/$2 ${APP_HOME}/${APP_FILENAME}
	is_suc "cp ~/JenkinsAutodeploy/update/$3/jar/$2 ${APP_HOME}/${APP_FILENAME}"
